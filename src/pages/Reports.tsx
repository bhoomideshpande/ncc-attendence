...
const Reports = () => {
  ...
  const filteredData = dummyStudents.filter((student) => {
    if (selectedInstitute !== "All" && student.institute !== selectedInstitute)
      return false;

    if (selectedBatch !== "All" && student.batch !== selectedBatch)
      return false;

    if (showAbsentOnly && student.attendance >= TOTAL_PARADES)
      return false;
                ...
                <Select onValueChange={setSelectedInstitute}>
                  ...
                  <SelectContent>
                    ...
                    {institutesList.map((inst, index) => (
                      <SelectItem key={index} value={inst}>{inst}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filteredData.map((student, i) => {
                  const absent = TOTAL_PARADES - student.attendance;

                  return (
                    <tr key={i} className="border-b">
                      <td className="p-3">{student.id}</td>
                      <td className="p-3 capitalize">{student.name}</td>
                      <td className="p-3">{student.institute}</td>
                      <td className="p-3">{student.batch}</td>
                      <td className="p-3 text-red-600 font-semibold">{absent} / 18</td>
                      <td className="p-3 text-green-600 font-semibold">{student.attendance} / 18</td>
                    </tr>
                  );
                })}
              </tbody>

...
router.post('/mark', async (req, res) => {
  try {
    ...
    if (parade) {
      parade.totalPresent = attendanceRecords.filter(r => r.status === 'present').length;
      parade.totalAbsent = attendanceRecords.filter(r => r.status === 'absent').length;
      parade.attendanceRecords = savedRecords.map(r => ({
...
router.get('/institute/:instituteCode/:date', async (req, res) => {
  try {
    ...
    }).populate('studentId', 'firstName lastName roll');
...
router.get('/report/:instituteCode', async (req, res) => {
  try {
    ...
    const attendance = await Attendance.find(query).populate('studentId', 'firstName lastName roll');

...
router.post('/seed-full', async (req, res) => {
  try {
    ...
    // Create institutes (or update if exists)
    for (const instituteData of institutes) {
      ...
      if (!existing) {
        ...
        // Update existing institute with new logo and totalCadets
        await existing.update({ logo: instituteData.logo, totalCadets: instituteData.totalCadets });
        console.log(`✓ Institute updated: ${instituteData.code}`);
      }
    }

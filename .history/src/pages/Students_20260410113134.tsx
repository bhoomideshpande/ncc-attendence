...
const Students = () => {
  ...
  React.useEffect(() => {
    const fetchStudents = async () => {
      try {
        // Get institute from localStorage (set during login)
        const institute = localStorage.getItem('institute');
        
        if (institute) {
          // Fetch students from the logged-in staff's institute
          const response = await studentService.getByInstitute(institute);
          setStudents(response.data);
        } else {
          // Fallback to all students if institute not found
